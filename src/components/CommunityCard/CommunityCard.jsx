...
export const CommunityCard = ({ community }) => {
  ...
  const { joinCommunity,isJoined , checkJoinedStatus} = useJoinCommunity();
      ...
      <div className={'border border-borderPrimary dark:border-borderSecondary my-2 hover:bg-softGrey dark:hover:bg-darkerGrey cursor-pointer  shadow p-4 rounded-lg'}>
        ...
        <div className={'flex justify-between items-center w-full'}>
          ...
          <div className={'w-[120px]'}>
            ...
            {community.createdBy !== user.uid && !isJoined &&
              <Button py={1} onClickHandler={()=>joinCommunity(community)}>Join</Button>
            }
            {isJoined &&
              <Button py={1} disabled={true} >Joined</Button>
            }
          </div>
        </div>
      </div>
